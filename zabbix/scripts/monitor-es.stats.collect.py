...
tmpfile = '/tmp/monitor_es_stats.tmp.%s' % (getpass.getuser())
...
def kvreturn(kv, kname, node_conf, f):
    for k, v in kv.items():
        if type(v) is dict:
            if kname == node_conf['nodename']:
                k = "%s,%s" % (kname, k)
            else:
                k = "%s-%s" % (kname, k)
            kvreturn(v, k, node_conf, f)
        else:
            line = "%s monitor-es-stats[%s,%s] %s\n" % (HOSTNAME, kname, k, v)
            f.write(line)
...
if __name__ == '__main__':
    ...
    for node, node_conf in monitor_es_conf.items():
        #get _cluster/health info
        cluster_health = get_data(node_conf['host'], node_conf['port'], '_cluster/health')
        kvreturn(cluster_health, node_conf['nodename'], node_conf, f)

        #get _nodes/stats/ info
        node_stats = get_data(node_conf['host'], node_conf['port'], '_nodes/stats/')
        for k, v in node_stats['nodes'].items():
            if v['name'] == node_conf['nodename']:
                #only send self data by default
                kvreturn(v, node_conf['nodename'], node_conf, f)
    f.close()
